...
| **内置资源池** | 143 个 Telegram 网盘频道（全部启用） + 3 个**原生插件源**（由 Worker 直接抓取，不依赖第三方站点） |
...
| # | 优化项 | 做法 |
| :-: | :--- | :--- |
| 1 | **搜索不再依赖第三方聚合节点** | 新增 `src/plugins/native.ts` —— 声明式**原生源引擎**，把资源站抓取直接跑在 Worker 里。原先唯一的内置插件是一个把 89 个子源全部外包出去的第三方节点（`so.252035.xyz`），实测它同一关键词三次返回 428 / 307 / 133 条，且**独有贡献只有 3.6%**，却是整条链路的单点故障。现在它保留在配置里但**默认停用**，想要可以随时在后台开回来。 |
| 2 | **三类适配器覆盖大部分资源站** | ① JSON POST 自研接口（影盘社，单关键词约 400 条）；② 苹果CMS `api.php/provide/vod`（链接塞在 `vod_down_url`，用 `$$$` 分隔）；③ Flarum 论坛 `/api/discussions`（链接在 `includes[].contentHtml` 里）。新增一个源通常只要 5~10 行配置。 |
| 3 | **插件源「一行 = 一个条目」** | 以前一个「插件」背后是 89 个子源，开关形同虚设（节点一次性全抓）。现在每个原生源各自独立抓取，后台逐行开关才有真实含义，并能逐个测试连通性。 |
| 4 | **新增插件源诊断** | `GET /api/debug/plugin?kw=…&ids=melost,ouge,quark4k` 返回**逐源**的状态码 / 条数 / 耗时（原生源跑 Worker 内引擎，第三方节点走 HTTP）。后台「测试已启用源」按钮用的就是它。 |
| 5 | **落地一条源前先验出口可达性** | 国内不少资源站把 Cloudflare 的 IP 段拉黑了，代码写得再对也拿不到数据：实测 `hunhepan` 直接回 `403 ip_blacklist`、`jikepan` 回 `530 error 1016`。用 `GET /api/debug/fetch?url=<接口>&full=1` 让 Worker 的出口 IP 去请求一次即可判定。 |
| 6 | **插件源数量上限 2 → 4** | 旧值 2 是给「一次要跑 5~8 秒的聚合节点」定的保守值。原生源彼此独立、打的是不同站点，并发跑的总耗时约等于最慢那个，因此不再压这么低。 |
| 7 | **版本号 V1.4** | 仍集中在 `src/version.ts`；`package.json` 需同步（回归脚本从它读期望版本号，漏改就会红）。 |

### V1.5 更新要点：第三方源「可删 + 可批量导入 + 开关是真的」

V1.4 把插件源改成一行一个条目之后，第三方节点的问题暴露出来：它的子源能被看见、能被勾选，却**删不掉**、也没法一次贴一批进去；更麻烦的是**勾选状态是假的**——节点名下的子源共用一次 HTTP，取消勾选并不会让请求少带这个源。

| # | 修复项 | 做法 |
| :-: | :--- | :--- |
| 1 | **第三方源可逐个删除** | 每行行尾加垃圾桶（桌面表格 + 移动卡片都有）。删的是「这一行」；把某节点的子源删空后保存，该节点配置整体移除，避免留下空节点。 |
| 2 | **按英文逗号批量导入** | 插件区底部新增批量导入框，英文逗号 / 换行 / 分号 / 空格都能分隔，也接受整条链接（自动取最后一段）。重复的自动跳过、非法字符自动清洗，默认「导入即启用」（旁边可取消）。导入的源挂到现有第三方节点下，一次贴 89 个也没问题。 |
| 3 | **子源开关变成真开关** | 新增 `PluginConfig.disabledPluginIds`：`pluginIds` 仍保留**全集**（否则关掉的源在后台就再也显示不出来、没法重新打开），请求前由 `effectivePluginIds()` 相减，只把真正启用的源发给节点。子源全关时**整个节点一次请求都不发**。⚠️ 生效的子源清单必须写进 URL 的 **`plugins=` 查询参数**——实测该聚合节点**完全忽略 `X-Plugins` 请求头**，早期用请求头下发等于没下发（详见第 9 条）。 |
| 4 | **后台「测试已启用源」按请求单元去测** | 界面上一行 ≠ 一次请求：原生源一行一次抓取，第三方节点名下 N 个子源共用一次 HTTP。以前测试按钮拿子源 id 去问后端，一个都匹配不上；现在先折成请求单元，响应还会带出「这次请求覆盖了哪几个子源」。 |
| 5 | **`/api/debug/plugin` 认子源 id** | 传节点 id 或它名下任意 `pluginIds` 都能命中同一个节点，返回值新增 `pluginIds` / `covered`。 |
| 6 | **`/api/health` 的 `plugins_enabled` 计入原生源** | 原先只按 `apiEndpoint` 计数，3 个原生源全开时它报 0，和 `/api/plugins` 的 `enabled` 对不上。 |
| 7 | **修一个静默失败的坑** | `resolvePansouParent` 曾在浏览器脚本里直接引用构建期的 `DEFAULT_PLUGINS`，抛 `ReferenceError` 却被 Vue 吞掉 → 点「确认导入」毫无反应。改为 Node 侧 `JSON.stringify` 注入。 |
| 8 | **端到端回归 `verify_plugin_rows_e2e.mjs`（59 项）** | 本地真服务 + 真 Chrome + **本地假聚合节点**（记录出站 `plugins=` 参数与请求头）：批量导入 → 删 1 行 → 取消勾选 1 行 → 保存 → 读回落盘配置核对 → 刷新页面还原 → 一次粘 88 个源 → 「回到内置默认」 → 频道批量导入（在 TG 频道页内导入 / 去重 / 拒非法名 / 落盘 / 确认已从「系统设置」移走）。并收集页面级 JS 报错，专治第 7 条那种被 Vue 吞掉的异常。 |
| 9 | **修掉「关了还在跑」的真根因：子源清单改走 `plugins=` 查询参数** | 上一版把生效子源拼进 `X-Plugins` 请求头，而 `so.252035.xyz` **只认 URL 上的 `plugins=`**：不带该参数的请求会返回节点自带的全部源，于是后台取消勾选的源照样出现在结果里（线上实测：已停用的 `gaoqing888` 仍回 107 条）。现改为 `&plugins=id1,id2`，并删掉无效请求头。线上隔离验证用 `verify_plugin_switch_live.mjs single clxiong`：把 89 个子源关到只剩 1 个，返回的插件来源**只有那 1 个源**。 |
| 10 | **「批量导入频道」从「系统设置」页挪到「TG 频道」页** | 原先埋在系统设置的角落，和频道列表隔着两个 Tab。现在放在频道搜索框下方同屏，改完直接点保存。分隔符与插件源批量导入统一（英文逗号 / 换行 / 分号 / 空格），可整段粘 `@name` 或 `t.me` 链接，自动去重、剔除非法名，并回显跳过数量。 |
...
> 📌 其中第 5 条（把 89 个子源拆成独立行、保存时再合并回一条聚合节点配置）已被 **V1.4 取代**：
> 那种「拆开显示、合并提交」的开关是**假的** —— 节点一次请求就把全部子源跑完了，
> 关掉其中一个并不会少发请求。V1.4 换成原生源后，一行才真的等于一次独立抓取。
...
- 🔌 **插件引擎（三类型）**：**原生源**（Worker 内直接抓资源站，默认启用、不依赖任何第三方）、`pansou` 兼容节点（默认停用，可换你自己的地址）、任意自定义 REST API（可配字段映射）；后台逐源启停、逐源测试。
...
│   ├── defaults.ts     # 内置 143 个 TG 频道默认配置 + 默认插件源（原生源默认启用、聚合节点默认停用）
│   ├── checker.ts      # 网盘链接失效检测（测活）判定引擎
│   ├── icons.ts        # 内联 SVG 图标（零外部图标库依赖）
│   ├── plugins/
│   │   ├── index.ts    # 插件引擎入口：按 type 分发到 native / pansou / custom
│   │   └── native.ts   # 原生源引擎：声明式适配器（JSON POST / 苹果CMS / Flarum），Worker 内直接抓取
...
├── verify_v13.mjs           # 密码哈希 / 插件逐源 / 前台门禁 / 旧配置迁移 / 多 isolate 可见性（97 项，离线）
├── verify_v13_live.mjs      # 线上验收（含后台保存链路与展示白名单，36 项；⚠️ 会真实改一次线上配置并还原）
├── verify_native_live.mjs   # 原生源线上验收（真浏览器，断言页面卡片数 / 分类 Tab 计数）
├── verify_plugin_compare.mjs # 插件侧产出与 so.252035.xyz 的对照基准（确认不因弃用而掉量）
├── verify_display_e2e.mjs   # 「结果展示选网盘」端到端回归（真服务 + 真浏览器 + 后台勾选交互，26 项）
...
- 把 `plugins` 恢复为**内置默认插件**（原生源全部启用、第三方聚合节点停用）
...
# 密码哈希 / 插件逐源开关 / 前台门禁 / 旧配置迁移 / 多 isolate 可见性（97 项，离线）
node build.mjs && node verify_v13.mjs

# 「结果展示选网盘」端到端回归：真服务 + 真浏览器 + 后台勾选交互（26 项，离线）
node verify_display_e2e.mjs

# V1.2 特性回归：PWA / 后台分页 / 网盘白名单 / KV 模式 / 版本一致性（40 项）
node verify_v12.mjs https://<你的域名>

# 线上验收（35 项）—— ⚠️ 会真实改一次线上「结果展示」白名单并还原，
# 跑完务必确认最后两行「还原成功（直读 KV 已确认）」「还原后前端配置同步」都是 PASS
node verify_v13_live.mjs https://<你的域名>

# 原生源线上验收：真浏览器里搜一次，断言页面卡片数与分类 Tab 计数
node verify_native_live.mjs https://<你的域名> 西游记

# 插件侧产出对照：确认弃用第三方节点后没掉量
node verify_plugin_compare.mjs https://<你的域名> 西游记 庆余年
```

`verify_full.mjs` 会模拟前端的「8 频道/片 × 4 路并发」调度跑满全量频道，输出每个关键词的结果总数、耗时与网盘分布。
`verify_v13.mjs` 是一次跑完全部后台/插件契约：后台不回传哈希、登录后自动升级旧明文、插件源逐行开关且提交顺序**原样落盘**（迁移不得在保存路径重跑）、空备注能清空、前台独立/复用两种密码模式与令牌失效、老 KV 配置读出来会补入原生源并把聚合节点停用，以及**「保存后立刻读取必须拿到最新配置」**（用 `import('dist/worker.js?case=N')` 造多个独立模块作用域模拟多 isolate）。
`verify_native_live.mjs` 专治「接口返回了但页面没有」：它不信任接口的条数，只以 DOM 上的 `.card-hover-effect` 卡片数与分类 Tab 计数为准，并用原生 `value` setter + `input` 事件驱动 Vue 的 v-model（直接赋值收不到）。**实测教训**：曾经页面只有 603 条而接口侧合计 1465 条，查到最后是 `verify_v13_live.mjs` 的还原步骤没落盘，线上白名单停在「仅夸克 + 阿里云盘」——不是业务 bug。
`verify_plugin_compare.mjs` 是 V1.4 的**下限护栏**：把原生源的产出与第三方节点 `so.252035.xyz` 同关键词对比，本站少于它就判失败。参照值（2026-09）：西游记 403 vs 299、庆余年 380 vs 114。
...
| 类型 | 说明 | 关键字段 |
| :--- | :--- | :--- |
| `native` | **默认启用**。Worker 内直接抓资源站，不经过任何第三方。实现见 `src/plugins/native.ts`，一类站点一个适配器 | `id`（对应引擎里注册的源）、`enabled` |
...
后台「`/admin` → 搜索插件」里**一行 = 一个插件源**，可逐个启停、写备注、点「测试已启用源」看每个源的真实状态码/条数/耗时。配置存入 KV 的 `plugins` 字段。

> **`native` 源为什么没有 `apiEndpoint`**：地址是写死在引擎里的适配器常量，不是配置项。判定「这个源可用」时要按 `type === 'native' || !!apiEndpoint` 来判，只看 endpoint 会把原生源全判成不可用。

### 内置原生源清单（实测于 2026-09）

| id | 名称 | 适配器 | 说明 |
| :--- | :--- | :--- | :--- |
| `melost` | 影盘社 | JSON POST 自研接口 | 产出最高的源，单关键词约 400 条（8 页 × 50） |
| `ouge` | 欧格资源 | 苹果CMS `api.php/provide/vod` | 片库直链，链接在 `vod_down_url` 用 `$$$` 分隔 |
| `quark4k` | 夸克4K | Flarum `/api/discussions` | 论坛帖，链接在 `includes[].contentHtml`（需去标签） |

实测过的坑（省得下次重踩）：

- **影盘社单页固定 50 条**：`size` 传 100 / 200 会被服务端压回 **10** 条（响应里的 `per_size` 会变），所以「加深」只能靠翻页。`type` 参数**不是筛选项**（传任何非空值都返回 0 条），该站天然 96% 是夸克。
- **影盘社在 Worker 出口可用、在本机不可用**：本机直连会撞上本地出口过滤器返回「URL过滤」页；反过来 `hunhepan` 等站在 Worker 出口被 `403 ip_blacklist`。**两边都测一次**才能分清是代码问题还是网络问题。
- 已排除的候选：`sousou.pro` 返回 JS 挑战页（需浏览器执行才能过）；`mikuclub.uk` 返回的是文章列表、网盘链接要到每篇文章里再抓（子请求预算撑不住）；`feikuai` 429、`huban` 1003、`hdr4k`/`panta` 1016（源站或 Cloudflare 侧异常）。
...
因此约定：
- **带 `channels` 参数的分片请求 → 不调插件**（否则插件会被打 18 遍）；
- 插件侧自己也分片：前端把已启用源按 **3 个一批**切成若干片，每片单独发一次
  `{"plugins": ["melost","ouge","quark4k"]}`，与频道分片并行、渐进式渲染；
- 结果在前端按 URL 去重合并。

> 为什么要分片：原生源每个都要翻好几页（影盘社 8 页 ≈ 6~8s）。全塞进一个请求会撞上
> Workers 的 50 子请求 / 6 并发连接上限，前面的源超时后，后面的源就再也跑不到了。
> 显式指定 `plugins` 的分片请求**不受** `maxPluginsPerSearch` 上限约束（上限只兜底「未指定」的调用方）。

### 上游 WAF 与高可用设计

> 这一节针对的是**第三方 `pansou` 聚合节点**（V1.4 起默认停用，仍可按需开回来）。
> 原生源不走这条路：它们直连资源站，失败模式是「源站封 Cloudflare 出口 IP」
> 或「源站自己挂了」，落地新源前务必先用 `GET /api/debug/fetch?url=<接口>&full=1` 验一次。
