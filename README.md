...
| 网盘 | 通道 | 依据 |
| :--- | :--- | :--- |
| 阿里云盘 | `api` | 官方匿名分享查询接口；`404` = 分享不存在 |
| 夸克网盘 | `api` | `sharepage/token` 接口；`code 0` 有效，`41027/41006/…` 失效，`41001/41002` 需提取码 |
| 百度网盘 | `api` → `page` | `wxlist` 接口；失败则回退页面特征词 |
| 123 网盘 | `api` → `page` | 官方分享接口；失败则回退页面特征词 |
| 天翼 / 115 / 迅雷 / PikPak / 光鸭 / 移动 / UC | `page` | 页面特征词匹配 |
